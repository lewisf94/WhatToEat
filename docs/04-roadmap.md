...
| **P6** | [e-ink display](plan/06-phase-eink-display.md) | `/api/display.png` renderer + ESPHome firmware for your chosen e-ink board (board TBD; front-runner: reTerminal E1001) | the kitchen display shows the top "eat me first" items and refreshes unattended |
...
- Hardware isn't needed until **P6**, and the board choice stays open until then — the display is swappable by design (server renders a PNG; firmware just fetches it). The reTerminal E1001 is only the best-verified front-runner, not a commitment.
