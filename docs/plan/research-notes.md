...
- **Seeed reTerminal E1001** — 7.5" mono, **800×480**, 4-level greyscale, **ESP32-S3**, **2000 mAh built-in, ~3-month battery** at a 6 h refresh, enclosure + buttons included, **$79**, **first-class ESPHome support with documented pins**. It's a solid *finished-board alternative* (Lewis chose the DIY XIAO C3 build instead) — verified pins/driver and no DIY battery/case work, so it's the drop-in swap if the DIY route ever palls.
...
- **Chosen build (Lewis): DIY XIAO ESP32-C3 + Waveshare 4.2″ (400×300)**, solar + 1000–1500 mAh LiPo + USB-C top-up; mounting spot = average room light (solar extends, doesn't fully sustain). XIAO C3: WiFi+BLE, ~44–80 µA sleep, on-board USB-C + TP4056 charging (~500 mA, 500–1500 mAh cells). ESPHome `board: esp32-c3-devkitm-1`; panel `waveshare_epaper` model `4.20in` (SPI mode 0), SPI SCK=GPIO8/MOSI=GPIO10, CS/DC/BUSY/RESET = builder's wiring.
- **C3 vs C6 vs H2 for this display** (the display needs **WiFi** to fetch the PNG): **C3** = WiFi+BLE, no 802.15.4 → ideal (cheapest, lowest-hassle, proven). **C6** = WiFi 6 + BLE + 802.15.4 → also works over WiFi, but the Thread radio is dead weight for an image-fetch display (can't send images over Thread), costs more, slightly higher active draw; only pick it to standardise on one chip. **H2** = 802.15.4 + BLE, **no WiFi** → **can't fetch the image at all**; it's a Thread/Zigbee *sensor* chip. → C3 for the display; C6/H2 belong on the P9f Thread sensors.
...
- 2026-07-20 (P4): No Docker **daemon** in the sandbox (CLI present, socket absent), so the image build + container run + Tailscale + Pi install are 🖐. Verified instead the **exact production runtime path on the host**: prod env + `WEB_DIST` (built PWA) + non-default `MIGRATIONS_DIR` + `AUTH_TOKEN` → health open, `/api/*` 401 without/‑wrong bearer and 200 with it, web shell served, seed via the env migrations dir, and the web client sending the token from `localStorage`. Choices: run the server via **`tsx`** in the container (not a tsc/esbuild build — avoids the workspace `.ts` dual-package hazard, fine for a personal add-on); install Tailscale by **copying the binaries from `tailscale/tailscale:stable`** (not `apk`) to dodge Alpine-repo uncertainty; skipped `build.yaml` (the `node:24-alpine`/`tailscale` bases are multi-arch, so the Dockerfile builds for the Pi's arch as-is). `tailscale serve` flag spelling is version-dependent — `run.sh` tries current + fallback forms and logs guidance.
- 2026-07-20 (P3): Self-hosted-wasm path **resolved and proven**: `import "zxing-wasm/reader/zxing_reader.wasm?url"` (Vite emits a hashed same-origin asset, precached by Workbox) + `setZXingModuleOverrides({ locateFile })` from `barcode-detector/pure` (it re-exports that from `zxing-wasm/reader`). Verified with a real decode — Chromium's fake camera fed a generated EAN-13 and the scanner filled the field, with the wasm fetched same-origin and **zero CDN requests**. PWA installability, offline shell, and the camera-denied fallback all pass. The on-device iPhone camera scan stays 🖐 (needs P4 HTTPS). Icons were rendered with Chromium (resvg's native binary isn't linked on this host).
