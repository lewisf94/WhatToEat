...
- **Chosen approach (Lewis): solar + LiPo backup + USB-C top-up** on a low-power board (Inkplate-class). One LiPo buffer; a solar charge input + the board's USB-C charger both feed it. Standard LiPo (not LiFePO4) keeps the built-in USB-C charging simple. Board + light level TBD (asked).
